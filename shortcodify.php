		...
		*/
		public function __call($name, $argumente) {
			...
			switch($name){
				...
				case 'createWidget':
					...
					$rtn = '<div class="shortcodify_widget sc_w'.$number.'">'.$html.'</div>';
				break;
                case 'youtube':
                    if( ! isset($atts['height']) ) $atts['height'] = 315;
                    if( ! isset($atts['width']) ) $atts['width'] = 560;
                    if( $content == '') $content = 'afzrRy2y5Lo';
                    $code = $content;
                    $rtn = '<iframe width="' . $atts['width'] . '" height="' . $atts['height'] . '" src="https://www.youtube.com/embed/' . $code . '?rel=0" frameborder="0" allowfullscreen></iframe>';
				break;
				default:
					$rtn = $content;
			}
		...
		function useTabs(){
		    //Register with hook 'wp_enqueue_scripts', which can be used for front end CSS and JavaScript
	     	add_action( 'wp_enqueue_scripts', array(&$this,'loadCss') );
			add_action( 'wp_enqueue_scripts', array(&$this,'loadJs') );
			add_shortcode( 'tabs', array(&$this, 'createTabs') );
			add_shortcode( 'tab', array(&$this, 'createTabstab') );
		}
		...
		function createTabs($atts, $content = NULL){
			//$atts['link']
			...
			// remove whitespace at beginning
			$ausgabe = preg_replace('/\A.*?\[tab/s', '[tab', $ausgabe);
			// remove whitespace at end of sting
			$ausgabe = preg_replace('/\A.*?\]noitces\/\[/s', ']noitces/[', strrev($ausgabe));
			$ausgabe = strrev($ausgabe);
			// remove whitespace between tags
			$ausgabe = preg_replace('/\[\/tab\].*?\[tab/s', '[/tab][tab', $ausgabe);
			
			$tabscontent = do_shortcode( $ausgabe );
			$rtn = '<div class="sc_tabs '.$name.'">'.
			 		'<ul>' . 
			 		$this->tabitem
			 		 .'</ul>'.
			 		$tabscontent.
		...
		function createTabstab($atts, $content = NULL){
			if(isset($atts['name'])) $name = $atts['name'];
			else $name = '';
			
			$this->tabnum++;
			
			$this->tabitem .= '<li><a href="#tabs-' . $this->tabnum . '">' . $name . '</a></li>';
			$rtn = '<div id="tabs-' . $this->tabnum . '"><p>'.do_shortcode($content).'</p></div>';
			return $rtn;
		}
		...
		function admin_options_page() {
			if (isset($_POST['wp_test_save'])) {
				if (wp_verify_nonce($_POST['_wpnonce'], 'wp-test-update-options')) {
					...
					$this->options['sc_tabs'] = (isset($_POST['sc_tabs']) && $_POST['sc_tabs'] === 'on') ? TRUE : FALSE;
